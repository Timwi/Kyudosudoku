window.onload = (function()
{
	...
	function validateConstraint(grid, constr)
	{
		switch (constr[':type'])
		{
			...
			case 'Means':
				function isArith(a, r, b) { return a + b == 2 * r; }
				function isGeom(a, r, b) { return a * b == r * r; }
				let numArith = 0, numGeom = 0;
				for (let triplet of [
					[constr.Cell - 10, constr.Cell, constr.Cell + 10],
					[constr.Cell - 9, constr.Cell, constr.Cell + 9],
					[constr.Cell - 8, constr.Cell, constr.Cell + 8],
					[constr.Cell - 1, constr.Cell, constr.Cell + 1]])
				{
					if (grid[triplet[0]] === null || grid[triplet[1]] === null || grid[triplet[2]] === null)
						return null;
				...
				return numArith == constr.NumArithmetic && numGeom == constr.NumGeometric;
			...
			case 'GermanWhisper': {
				let anyNull = false;
				for (let i = 1; i < constr.Cells.length; i++)
				{
					if (grid[constr.Cells[i - 1]] === null || grid[constr.Cells[i]] === null)
						anyNull = true;
					else if (Math.abs(grid[constr.Cells[i - 1]] - grid[constr.Cells[i]]) < 5)
						return false;
				}
				return anyNull ? null : true;
			}
	...
	Array.from(document.getElementsByClassName('puzzle')).forEach(puzzleDiv =>
	{
		...
		function dbUpdate(isSolved)
		{
			...
			req.onload = function()
			{
				...
				if (json)
				{
					...
					var t1 = puzzleDiv.querySelector('.solve-time-1');
					var t2 = puzzleDiv.querySelector('.solve-time-2');
					var t3 = puzzleDiv.querySelector('.solve-time-3');
					t1.setAttribute('transform', '');
					t2.setAttribute('transform', '');
					t3.setAttribute('transform', '');

					var bbt = puzzleDiv.querySelector('.solve-text').getBBox();
					var bb1 = t1.getBBox();
					var bb2 = t2.getBBox();
					var bb3 = t3.getBBox();
		...
		function updateVisuals(udpateStorage)
		{
			...
			if (isSolved)
			{
				let noSolveTime = puzzleDiv.classList.contains('no-solve-time');
				puzzleDiv.querySelector('.solve-time').setAttribute('visibility', noSolveTime ? 'hidden' : 'inherit');
				puzzleDiv.querySelector('.no-solve-time').setAttribute('visibility', noSolveTime ? 'inherit' : 'hidden');
				dbUpdate(true);
			}
